...
def export_xlsx_keywords(data, start_date, end_date):
    extension = 'xlsx'
    ...
    worksheet.write(0, 1, 'Repetitions')
    worksheet.write(0, 2, 'Sentiment')

    # Write the review data to the worksheet
    for row, review in enumerate(data, start=3):
        worksheet.write(row, 0, review['word'])
        worksheet.write(row, 1, review['number_of_repetitions'])
        worksheet.write(row, 2, review['sentiment'])
    ...
    chart.add_series({
        ...
        'values': '=Sheet1!$B$4:$B$' + str(len(data) + 3),
    })
    ...
    chart.set_y_axis({'name': 'Keywords Repetitions'})
...
def export_pdf_keywords(data, start_date, end_date):
    header = 'Keywords Impact'
    ...
    pdf_chart_page_keywords(p, logo, title_chart, data, header)
    draw_standard_logo(p, logo)
    cols = [
        {'name': 'Word', 'param': 'word', 'spacing': 50},
        {'name': 'Repetitions', 'param': 'number_of_repetitions', 'spacing': 250},
        {'name': 'Sentiment', 'param': 'sentiment', 'spacing': 400},
    ]
    write_table_header_keywords(p, cols)
...
def pdf_chart_page_keywords(p, logo, title_chart, data, header):
    p.setFont('NotoSans-Bold', 14)
    x = letter[0] / 2
    y = letter[1] / 2
    draw_standard_logo(p, logo)
    ...
    labels = [entry['word'] for entry in data]
    values_total_reviews = [entry['number_of_repetitions'] for entry in data]
    ...
    plt.bar(index, values_total_reviews, bar_width, color='blue', label='Repetition Keywords')
...
def write_table_header_keywords(p, cols):
    page_height = 800  # Maximum y coordinate for the page content
    row_height = 20  # Height of each row
    y = page_height - row_height
    p.setFont('NotoSans-Bold', 12)
