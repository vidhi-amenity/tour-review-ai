...
def export_xlsx_comparative(data, start_date, end_date):
    extension = 'xlsx'
    ...
    worksheet.write(0, 0, 'Tour')
    worksheet.write(0, 1, 'Stream')
    worksheet.write(0, 2, 'Positive')
    worksheet.write(0, 3, 'Passive')
    worksheet.write(0, 4, 'Negative')

    # Write the review data to the worksheet
    for row, review in enumerate(data, start=3):
        worksheet.write(row, 0, review['tour'])
        worksheet.write(row, 1, review['agency'])
        worksheet.write(row, 2, review['positive'])
        worksheet.write(row, 3, review['passive'])
        worksheet.write(row, 4, review['negative'])
    ...
    chart.add_series({
        ...
        'values': '=Sheet1!$C$4:$C$' + str(len(data) + 3),
    })
...
def export_pdf_comparative(data, start_date, end_date):
    header = 'Comparative Graphics'
    ...
    pdf_chart_page_comparative(p, logo, title_chart, data, header)
    draw_standard_logo(p, logo)
    cols = [
        {'name': 'Tour', 'param': 'tour', 'spacing': 50},
        {'name': 'Stream', 'param': 'agency', 'spacing': 250},
        {'name': 'Positive', 'param': 'positive', 'spacing': 350},
        {'name': 'Passive', 'param': 'passive', 'spacing': 420},
        {'name': 'Negative', 'param': 'negative', 'spacing': 490},
    ]
    write_table_header_comparative(p, cols)
...
def pdf_chart_page_comparative(p, logo, title_chart, data, header):
    p.setFont('NotoSans-Bold', 14)
    x = letter[0] / 2
    y = letter[1] / 2
    draw_standard_logo(p, logo)
    ...
    labels = [entry['tour'] for entry in data]
    values_total_reviews = [entry['positive'] for entry in data]
...
def write_table_header_comparative(p, cols):
    page_height = 800  # Maximum y coordinate for the page content
    row_height = 20  # Height of each row
    y = page_height - row_height
    p.setFont('NotoSans-Bold', 12)
