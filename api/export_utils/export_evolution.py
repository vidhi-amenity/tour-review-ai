...
def export_xlsx_evolution(data, start_date, end_date):
    extension = 'xlsx'
    ...
    for row, review in enumerate(data, start=3):
        worksheet.write(row, 0, review['date'])
        worksheet.write(row, 1, review['value'])

    # Create a new chart object and add data to it
    chart = workbook.add_chart({'type': 'column'})
    chart.add_series({
        'name': 'Reviews Average',
...
def export_pdf_evolution(data, start_date, end_date):
    header = 'Summary Evolution'
    ...
    pdf_chart_page_evolution(p, logo, title_chart, data, header)
    draw_standard_logo(p, logo)
    cols = [
        {'name': 'Date', 'param': 'date', 'spacing': 50},
        {'name': 'Mean', 'param': 'value', 'spacing': 250}
    ]
    write_table_header_evolution(p, cols)
...
def pdf_chart_page_evolution(p, logo, title_chart, data, header):
    p.setFont('NotoSans-Bold', 14)
    x = letter[0] / 2
    y = letter[1] / 2
    draw_standard_logo(p, logo)
    ...
    plt.bar(index, values_total_reviews, bar_width, color='blue', label='Average')

    plt.xlabel(header)
    plt.ylabel('Reviews')
    plt.title('Reviews Average')
...
def write_table_header_evolution(p, cols):
    page_height = 800  # Maximum y coordinate for the page content
    row_height = 20  # Height of each row
    y = page_height - row_height
    p.setFont('NotoSans-Bold', 12)
